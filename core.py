...
class PathFinder:
    ...
    def _heuristic(self, current: Tuple[str, str], goal: Tuple[str, str]) -> float:
        """Hub-based heuristic for A* algorithm"""
        # If both are hubs, use precomputed hub distance if available
        if current[0] in self.planner.hub_stations and goal[0] in self.planner.hub_stations:
            if current in self.planner.hub_distances and goal in self.planner.hub_distances[current]:
               return self.planner.hub_distances[current][goal][0]
        
        # Otherwise, use sum of distances to nearest hubs as lower bound
